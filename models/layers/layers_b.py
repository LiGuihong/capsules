...
def _squash_sj(input_tensor):
  ...
  """
  with tf.name_scope('sj_norm_non_linearity'):
...
def _update_routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,
  ...
  print(votes_trans)
  print(r_t_shape)
  print_r_t_shape=tf.Print(r_t_shape,[r_t_shape],summarize=20)
  print_votes=tf.Print(votes_t_shape,[votes_t_shape],summarize=20) 
...
#    if i>0:
#      x=tf.fill(logit_shape,0.1)
#      two=tf.fill(logit_shape,1)
#      half=tf.fill(logit_shape,0.5)
#      route=tf.subtract(route,x)
#      route=tf.sign(route)
#      route=tf.add(route,two)
#      route=tf.multiply(route,halflogit_shape,1)
    ...
    logits += distances
#    print_mask=tf.Print(logits,[logits],summarize=1000000000)
#    logits=print_mask
...
def _new_routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,
                    num_routing, leaky):
  votes_t_shape = [3, 0, 1, 2]
  for i in range(num_dims - 4):
    votes_t_shape += [i + 4]
  ...
  for i in range(num_dims - 4):
    r_t_shape += [i + 4]
  votes_trans = tf.transpose(votes, votes_t_shape) 
#  c = tf.fill(logit_shape, 1.0)
#  preactivate=c*votes_trans
#  preactivate = tf.reduce_sum(votes_trans, axis=2)
  preactivate = tf.transpose(votes_trans, r_t_shape)
  preactivate = tf.reduce_sum(preactivate, axis=1)+biases
#  preactivate=tf.transpose(preactivate)+biases
#  preactivate = tf.reduce_sum(votes_trans, axis=1) + biases
  activation = _squash_sj(preactivate)
  # distances: [batch, input_dim, output_dim]
  act_3d = tf.expand_dims(activation, 1)
  tile_shape = np.ones(num_dims, dtype=np.int32).tolist()
  tile_shape[1] = input_dim
